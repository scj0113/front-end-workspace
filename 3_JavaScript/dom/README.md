...
- 두번쨰 : 창 이름, 창 이름이 같은게 이미 열려 있을 경우 새로 열리지 않고 이미 열려있는 곳에서 새로고침
...
```js
doument.getElementsByClassName("클래스");
```
...
- 동일한 태그가 여러 개 존재할 수 있기 떄문에 배열로 리턴
...
- data-xxx : data-로 시작하는 표준 커스텀 속성.
  date-index, data-id, data-role 등 data-의 형식으로 시작하면 어떤 속성이든 필요에 따라 임의로 추가할 수 있다.
