...
```css
.container {
    gird-template-columns: 200px 500px;
...
- column의 개수를 미리 정하지 않고 설정된 너비가 허용하는 한 최대한 셀을 재움
- auto-fill은 셀의 개수가 모라자면 공간이 남지만, auto-fit은 채움
...
## grid-column-start, grid-column-end, gird-column, gird-row-start, grid-row-end, grid-row
...
```css
선택자 {
    backface-visibillity : visible or hidden;
}
```
